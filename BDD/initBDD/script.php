...
//Connect to BDD
try {
  $bdd = new PDO("mysql:host=localhost", "root", "123456",
...
$sql = "CREATE SCHEMA IF NOT EXISTS `fond_de_placard` DEFAULT     CHARACTER SET utf8 ;
USE `fond_de_placard`" ;
...
$sql ="CREATE TABLE IF NOT EXISTS `fond_de_placard`.`picture` (
...
$sql ="CREATE TABLE IF NOT EXISTS `fond_de_placard`.`recipe` (
...
$sql ="CREATE TABLE IF NOT EXISTS `fond_de_placard`.`group` (
...
$sql ="CREATE TABLE IF NOT EXISTS `fond_de_placard`.`link_rec_use` (
  `recipe_rec_id` INT NOT NULL COMMENT '',
...
$sql ="CREATE TABLE IF NOT EXISTS `fond_de_placard`.`comment` (
...
$sql ="CREATE TABLE IF NOT EXISTS `fond_de_placard`.`theme` (
...
$sql ="CREATE TABLE IF NOT EXISTS `fond_de_placard`.`link_rec_the` (
  `recipe_rec_id` INT NOT NULL COMMENT '',
...
//Ajout des ingredients
  ...
  $listIng = array(
    ...
    // Préparation de la requete sql pour ajouter les ingredients du tableau
    $insertIng = $bdd->prepare('INSERT INTO ingredients (ing_name)
    VALUES (:ing_name)');

    // Création d'une boucle pour ajouter les ingredients du tableau dans la base de donnée
    for ($i=0; $i < count($listIng) ; $i++) {
      $insertIng->execute(array(
        "ing_name" => $listIng[$i]
      ));
    }
  ...
  //Ajout des 3 groupes defini (utilsiateur par default(1), moderateur et admin)
    ...
    // Groupe utilisateur
    $sql = "INSERT INTO `group`(`gro_id`, `gro_name`) VALUES (1, 'utilisateur')";

    $bdd->exec($sql);

    //Groupe moderateur
    $sql = "INSERT INTO `group`(`gro_id`, `gro_name`) VALUES (2, 'moderateur')";

    $bdd->exec($sql);

    //Groupe admin
    $sql = "INSERT INTO `group`(`gro_id`, `gro_name`) VALUES (3, 'admin')";
